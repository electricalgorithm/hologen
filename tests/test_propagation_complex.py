...
class TestPlaneWavePropagation:
    ...
    def test_plane_wave_phase_shift(self) -> None:
        ...
        expected_phase = (
            2 * np.pi * optics.propagation_distance / optics.wavelength
        ) % (2 * np.pi)
    ...
    def test_zero_distance_no_change(self) -> None:
        ...
        field = np.random.rand(grid.height, grid.width) + 1j * np.random.rand(
...
class TestAmplitudeConservation:
    ...
    def test_parseval_theorem(self) -> None:
        ...
        np.random.seed(42)
        field = (
            np.random.rand(grid.height, grid.width)
            + 1j * np.random.rand(grid.height, grid.width)
    ...
    def test_energy_conservation_back_propagation(self) -> None:
        ...
        center_y, center_x = grid.height // 2, grid.width // 2
        field = np.exp(
            -((y - center_y) ** 2 + (x - center_x) ** 2) / (2 * 8.0**2)
        ).astype(np.complex128)
...
class TestComplexVsIntensityOnly:
    ...
    def test_round_trip_propagation(self) -> None:
        ...
        center_y, center_x = grid.height // 2, grid.width // 2
        field = np.exp(
            -((y - center_y) ** 2 + (x - center_x) ** 2) / (2 * 5.0**2)
        ).astype(np.complex128)
        ...
        backward = angular_spectrum_propagate(
            forward, grid, optics, -optics.propagation_distance
        )
