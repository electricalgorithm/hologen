...
class TestAngularSpectrumPropagate:
    ...
    def test_shape_validation(self, grid_spec: GridSpec, optical_config: OpticalConfig) -> None:
    ...
    def test_zero_distance_identity(self, grid_spec: GridSpec, optical_config: OpticalConfig) -> None:
        ...
        result = angular_spectrum_propagate(field, grid_spec, optical_config, distance=0.0)
        np.testing.assert_array_equal(result, field)

    def test_output_dtype(self, grid_spec: GridSpec, optical_config: OpticalConfig) -> None:
        """Test that output has correct dtype."""
        field = np.ones((grid_spec.height, grid_spec.width), dtype=np.complex64)
        result = angular_spectrum_propagate(field, grid_spec, optical_config, distance=0.01)
        assert result.dtype == np.complex128

    def test_output_shape(self, grid_spec: GridSpec, optical_config: OpticalConfig) -> None:
        """Test that output shape matches input."""
        field = np.ones((grid_spec.height, grid_spec.width), dtype=np.complex128)
        result = angular_spectrum_propagate(field, grid_spec, optical_config, distance=0.01)
        assert result.shape == field.shape

    def test_energy_conservation_plane_wave(self, optical_config: OpticalConfig) -> None:
    ...
    def test_reciprocity(self, grid_spec: GridSpec, optical_config: OpticalConfig) -> None:
        ...
        backward = angular_spectrum_propagate(forward, grid_spec, optical_config, -distance)
        
        np.testing.assert_allclose(backward, field, rtol=1e-12)

    def test_negative_distance(self, grid_spec: GridSpec, optical_config: OpticalConfig) -> None:
    ...
    def test_dc_component_preservation(self, grid_spec: GridSpec, optical_config: OpticalConfig) -> None:
    ...
    def test_large_distance_stability(self, grid_spec: GridSpec, optical_config: OpticalConfig) -> None:
        """Test numerical stability for large propagation distances."""
        field = np.ones((grid_spec.height, grid_spec.width), dtype=np.complex128)
        large_distance = 1.0  # 1 meter
        
        result = angular_spectrum_propagate(field, grid_spec, optical_config, large_distance)
