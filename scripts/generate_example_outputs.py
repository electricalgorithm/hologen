...
def generate_basic_visualization(output_dir: Path):
    ...
    print(f"  Saved: basic_visualization.png")
...
def generate_parameter_study_plot(output_dir: Path):
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'parameter_study_distance.png', dpi=150, bbox_inches='tight')
    plt.close()
    
    print(f"  Saved: parameter_study_distance.png")
...
def generate_ablation_study_plot(output_dir: Path):
    ...
    conditions = ['no_noise', 'sensor_only', 'speckle_only', 'aberration_only', 'all_noise']
    ...
    ax1.set_title('Effect of Noise on Reconstruction Quality', fontsize=14, fontweight='bold')
    ...
    print(f"  Saved: ablation_study_noise.png")
...
def generate_quality_distribution_plot(output_dir: Path):
    ...
    plt.axvline(np.mean(mse_values), color='red', linestyle='--', linewidth=2,
...
def generate_augmentation_example(output_dir: Path):
    ...
    axes[1].set_title('Augmented Hologram\n(Flipped, Rotated, Noisy)', 
                     fontsize=14, fontweight='bold')
    axes[1].axis('off')
