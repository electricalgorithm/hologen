...
def create_conversion_flow_diagram():
    ...
    func_style = dict(boxstyle='round,pad=0.2', facecolor='lightyellow', 
                     edgecolor='orange', linewidth=1.5)
    
    # Draw main nodes
    ax.text(positions['complex'][0], positions['complex'][1], 
            'Complex Field\n(real + imag)', 
            ha='center', va='center', fontsize=12, fontweight='bold',
            bbox=node_style)
    
    ax.text(positions['intensity'][0], positions['intensity'][1], 
            'Intensity\n|E|²', 
            ha='center', va='center', fontsize=11,
            bbox=node_style)
    
    ax.text(positions['amplitude'][0], positions['amplitude'][1], 
            'Amplitude\n|E|', 
            ha='center', va='center', fontsize=11,
            bbox=node_style)
    
    ax.text(positions['phase'][0], positions['phase'][1], 
            'Phase\narg(E)', 
            ha='center', va='center', fontsize=11,
            bbox=node_style)
    
    ax.text(positions['amp_phase'][0], positions['amp_phase'][1], 
            'Amplitude + Phase\n(separate arrays)', 
            ha='center', va='center', fontsize=11,
            bbox=node_style)
    ...
    ax.annotate('', xy=(positions['intensity'][0]+0.5, positions['intensity'][1]+0.3),
                xytext=(positions['complex'][0]-0.8, positions['complex'][1]-0.5),
                arrowprops=arrow_style)
    ax.text(3, 6.8, 'complex_to_representation\n(INTENSITY)', 
            fontsize=8, ha='center', bbox=func_style)
    
    # Complex to Amplitude
    ax.annotate('', xy=(positions['amplitude'][0], positions['amplitude'][1]+0.5),
                xytext=(positions['complex'][0], positions['complex'][1]-0.5),
                arrowprops=arrow_style)
    ax.text(5, 6.5, 'complex_to_representation\n(AMPLITUDE)', 
            fontsize=8, ha='center', bbox=func_style)
    
    # Complex to Phase
    ax.annotate('', xy=(positions['phase'][0]-0.5, positions['phase'][1]+0.3),
                xytext=(positions['complex'][0]+0.8, positions['complex'][1]-0.5),
                arrowprops=arrow_style)
    ax.text(7, 6.8, 'complex_to_representation\n(PHASE)', 
            fontsize=8, ha='center', bbox=func_style)
    
    # amplitude_phase_to_complex arrow
    reverse_arrow_style = dict(arrowstyle='->', lw=2, color='darkred')
    ax.annotate('', xy=(positions['complex'][0], positions['complex'][1]-0.5),
                xytext=(positions['amp_phase'][0], positions['amp_phase'][1]+0.5),
                arrowprops=reverse_arrow_style)
    ax.text(5.8, 5, 'amplitude_phase_to_complex', 
            fontsize=8, ha='center', bbox=func_style, rotation=90)
    
    # Add title
    ax.text(5, 9.5, 'Field Conversion Flow Diagram', 
            fontsize=16, fontweight='bold', ha='center')
    
    # Add legend
    legend_elements = [
        mpatches.Patch(facecolor='lightblue', edgecolor='darkblue', 
                      label='Field Representation'),
        mpatches.Patch(facecolor='lightyellow', edgecolor='orange', 
                      label='Conversion Function'),
        mpatches.FancyArrow(0, 0, 1, 0, width=0.1, color='darkgreen', 
                           label='Forward Conversion'),
        mpatches.FancyArrow(0, 0, 1, 0, width=0.1, color='darkred', 
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'conversion_flow_diagram.png', dpi=150, bbox_inches='tight')
    plt.close()
...
def create_file_format_examples():
    ...
    """
    ax.text(0.05, 0.95, format_text, transform=ax.transAxes,
...
def create_io_workflow_diagram():
    ...
    # Title
    ax.text(5, 9.5, 'Dataset I/O Workflow', 
            fontsize=16, fontweight='bold', ha='center')
    ...
    # Node styles
    process_style = dict(boxstyle='round,pad=0.3', facecolor='lightgreen', 
                        edgecolor='darkgreen', linewidth=2)
    storage_style = dict(boxstyle='round,pad=0.3', facecolor='lightgray', 
                        edgecolor='black', linewidth=2)
    model_style = dict(boxstyle='round,pad=0.3', facecolor='lightcoral', 
                      edgecolor='darkred', linewidth=2)
    
    # Draw nodes
    ax.text(positions['generator'][0], positions['generator'][1], 
            'Dataset\nGenerator', 
            ha='center', va='center', fontsize=11, fontweight='bold',
            bbox=process_style)
    
    ax.text(positions['writer'][0], positions['writer'][1], 
            'Writer\n(NumpyDatasetWriter\nor ComplexFieldWriter)', 
            ha='center', va='center', fontsize=10, fontweight='bold',
            bbox=process_style)
    
    ax.text(positions['disk'][0], positions['disk'][1], 
            'Disk Storage\n(.npz + .png files)', 
            ha='center', va='center', fontsize=11, fontweight='bold',
            bbox=storage_style)
    
    ax.text(positions['loader'][0], positions['loader'][1], 
            'Data Loader\n(load_complex_sample\nor custom loader)', 
            ha='center', va='center', fontsize=10, fontweight='bold',
            bbox=process_style)
    
    ax.text(positions['ml_model'][0], positions['ml_model'][1], 
            'ML Model\n(PyTorch/TensorFlow)', 
            ha='center', va='center', fontsize=11, fontweight='bold',
            bbox=model_style)
    ...
    # Generator to Writer
    ax.annotate('', xy=(positions['writer'][0]-0.8, positions['writer'][1]),
                xytext=(positions['generator'][0]+0.8, positions['generator'][1]),
                arrowprops=arrow_style)
    ax.text(3.5, 7.5, 'samples', fontsize=9, ha='center', 
            bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
    
    # Writer to Disk
    ax.annotate('', xy=(positions['disk'][0]-0.8, positions['disk'][1]),
                xytext=(positions['writer'][0]+0.8, positions['writer'][1]),
                arrowprops=arrow_style)
    ax.text(6.5, 7.5, 'save()', fontsize=9, ha='center',
            bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
    
    # Disk to Loader
    ax.annotate('', xy=(positions['loader'][0], positions['loader'][1]+0.5),
                xytext=(positions['disk'][0], positions['disk'][1]-0.5),
                arrowprops=arrow_style)
    ax.text(7, 5.5, 'load files', fontsize=9, ha='center',
            bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
    
    # Loader to ML Model
    ax.annotate('', xy=(positions['ml_model'][0], positions['ml_model'][1]+0.5),
                xytext=(positions['loader'][0], positions['loader'][1]-0.5),
                arrowprops=arrow_style)
    ax.text(5.8, 2.5, 'batches', fontsize=9, ha='center',
            bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
    
    # Add annotations
    annotation_style = dict(fontsize=8, style='italic', color='gray')
    ax.text(2, 6.2, 'Generate synthetic\nhologram samples', ha='center', **annotation_style)
    ax.text(5, 6.2, 'Persist to disk with\noptional previews', ha='center', **annotation_style)
    ax.text(8, 6.2, 'NumPy archives +\nPNG previews', ha='center', **annotation_style)
    ax.text(5, 3.2, 'Load and preprocess\nfor training', ha='center', **annotation_style)
    ax.text(5, 0.2, 'Train neural network\non synthetic data', ha='center', **annotation_style)
...
def create_normalization_example():
    ...
    axes[2].set_title(f'8-bit PNG\nRange: [{uint8_image.min()}, {uint8_image.max()}]', 
                     fontsize=11)
    axes[2].axis('off')
...
def create_phase_validation_example():
    ...
    axes[1].set_title('Invalid Phase\nRange: [-2π, 2π]\n✗ Fails validation', 
    ...
    axes[2].set_title('Phase with NaN\nContains non-finite values\n✗ Fails validation', 
                     fontsize=11, color='red')
    axes[2].axis('off')
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'phase_validation_example.png', dpi=150, bbox_inches='tight')
    plt.close()
