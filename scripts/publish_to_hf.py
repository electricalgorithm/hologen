...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Target Hugging Face Repo ID (e.g. username/dataset_name)",
    )
    parser.add_argument(
        "--dataset_path",
        type=str,
        default="dataset-224",
        help="Local path to the dataset folder",
    )
    parser.add_argument(
        "--private", action="store_true", help="Make the repository private"
    )
    parser.add_argument(
        "--token", type=str, help="Hugging Face token (optional if logged in via CLI)"
    )
    ...
    print(
        f"Uploading files from {args.dataset_path} to {args.repo_id} using upload_large_folder..."
    )
    try:
