...
def save_field_comparison(
    field: np.ndarray, output_path: Path, title: str = "Field Comparison"
) -> None:
    ...
    amplitude_normalized = (
        amplitude / amplitude.max() if amplitude.max() > 0 else amplitude
    )
...
def save_object_type_comparison(output_dir: Path) -> None:
    ...
    phase_sample = producer.generate_complex(
...
def save_hologram_reconstruction_example(output_dir: Path) -> None:
    ...
    # Generate sample
    rng = np.random.default_rng(42)
    object_sample = producer.generate_complex(
        config.grid, rng, phase_shift=np.pi / 2, mode="phase"
    )
...
def save_side_by_side_comparison(output_dir: Path) -> None:
    ...
    plt.tight_layout()
    plt.savefig(
        output_dir / "intensity_vs_complex_comparison.png", dpi=150, bbox_inches="tight"
    )
    plt.close()
